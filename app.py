...
# e5 임베딩 모델 로드
model_path = "./e5/e5/multilingual-e5-small"  # 로컬 경로
tokenizer = AutoTokenizer.from_pretrained(model_path)
model = AutoModel.from_pretrained(model_path)

# SQLite DB 경로
db_path = "/mnt/data/prugio_notice.db"
...
def get_embedding(text: str):
    ...
    return embeddings.squeeze()  # 차원 축소
...
def find_similar_entries(user_embedding, db_path, top_n=3, min_similarity=0.5):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT id, content, embedding FROM recipe")  # 테이블 확인 필요
    ...
    for recipe_id, content, embedding in cursor.fetchall():
        if embedding is None:
            continue
        try:
            embedding_array = np.frombuffer(embedding, dtype=np.float32)  # 바이트 -> numpy
...
async def main(message: cl.Message):
    ...
    if similar_entries:
        context = similar_entries[0]["content"]  # 가장 유사한 결과의 내용
    else:
